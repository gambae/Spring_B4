                 ...
                 <table class="commentView">
	                ...
	                <% for(int i=0; i<list.size(); i++) { %>
	                	<% if (list.get(i).getDepth().equals("0")) { %>
		                    <tr  style="background-color: #f0f4f4;">
		                    	<td style="width: 300px;"><%=list.get(i).getContent() %> <a href="cmtcmt.jsp?post_id=<%=post_id%>&title=<%=title%>&writer=<%=writer%>&content=<%=content%>&post_date=<%=post_date%>&views=<%=views%>&board_type=<%=board_type%>&comment_id=<%=list.get(i).getComment_id() %>" style="font-size:0.5em;">┖답글</a></td>
		                        <td style="width: 30px;"><%=list.get(i).getComment_writer()%></td>
		                       	<td style="width: 100px;">
		                        <% if (vo.getId().equals(list.get(i).getComment_writer())) { %>
		                        	<button style="padding: 10px; margin-right: 5px;"onclick="location.href='commentUpdate.jsp?comment_id=<%=list.get(i).getComment_id()%>&post_id=<%=post_id%>&title=<%=title%>&writer=<%=writer%>&content=<%=content%>&post_date=<%=post_date%>&views=<%=views%>&board_type=<%=board_type%>'">수정</button>
	                    ...
	                    <% } else {%>
		                    <tr style="background : #dcdcdc;">
		                    	<td style="width: 250px;">&emsp;&emsp;└─&emsp;<%=list.get(i).getContent() %> <a href="cmtcmt.jsp?post_id=<%=post_id%>&title=<%=title%>&writer=<%=writer%>&content=<%=content%>&post_date=<%=post_date%>&views=<%=views%>&board_type=<%=board_type%>&comment_id=<%=list.get(i).getComment_id() %>" style="font-size:0.5em;"></a></td>
